...
function Line({ data }) {
  ...
  const validData = uniqueData.filter(item => 
    ...
    item.parameter === 'tenantB.deviceBar.datasourceD.ipsum.sheep' &&
    item.timestamp &&
  ...
  const textData = uniqueData.filter(item => 
    item.device_name === "device.bar" &&
    item.parameter === 'tenantB.deviceBar.datasourceD.ipsum.chicken.licken' &&
    item.type === 'text' &&
    item.timestamp &&
    !isNaN(new Date(item.timestamp).getTime())
  );
  ...
  return (
    <div>
      ...
      <h1>Datasource C</h1>
      <h2>Sheep Time Series</h2>
      <div style={{ height: '420px' }}>
        <LineChart data={chartData} options={chartOptions} />
      </div>
      <table>
        <thead>
        <h2>Chicken Licken Time Series</h2>
          <tr>
            <th>Timestamp</th>
            <th>Text Value</th>
          </tr>
