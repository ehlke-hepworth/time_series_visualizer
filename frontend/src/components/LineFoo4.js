...
function Line({ data }) {
  ...
  const validData = uniqueData.filter(item => 
    item.device_name === "device.foo" &&
    item.parameter === 'tenantA.deviceFoo.datasourceD.ipsum.qux' &&
    item.timestamp &&
  ...
  const textData = uniqueData.filter(item => 
    item.device_name === "device.foo" &&
    item.parameter === 'tenantA.deviceFoo.datasourceD.ipsum.baz' &&
    item.type === 'text' &&
    item.timestamp &&
    !isNaN(new Date(item.timestamp).getTime())
  );
  ...
  const windowSize = 15; // Size of the moving average window
  ...
  return (
    <div>
      ...
      <h1>Datasource D</h1>
      <h2>QUX Time Series</h2>
      <div style={{ height: '420px' }}>
        <LineChart data={chartData} options={chartOptions} />
      </div>
      <table>
        <thead>
        <h2>Baz Time Series</h2>
          <tr>
            <th>Timestamp</th>
            <th>Text Value</th>
          </tr>
