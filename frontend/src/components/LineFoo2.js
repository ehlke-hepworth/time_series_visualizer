...
function Line({ data }) {
  ...
  const validData = uniqueData.filter(item => 
    ...
    item.parameter === 'tenantA.deviceFoo.datasourceB.garply' &&
    item.timestamp &&
  ...
  const textData = uniqueData.filter(item => 
    item.device_name === "device.foo" &&
    item.parameter === 'tenantA.deviceFoo.datasourceB.grault' &&
    item.type === 'text' &&
    item.timestamp &&
    !isNaN(new Date(item.timestamp).getTime())
  );
  ...
  return (
    <div>
      ...
      <h1>Datasource B</h1>
      <h2>Garply Time Series</h2>
      <div style={{ height: '420px' }}>
        <LineChart data={chartData} options={chartOptions} />
      </div>
      <table>
        <thead>
        <h2>Grault Time Series</h2>
          <tr>
            <th>Timestamp</th>
            <th>Text Value</th>
          </tr>
